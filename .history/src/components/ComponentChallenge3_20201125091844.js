...
function ComponentChallenge3() {
  const handleTask = () => {
    ...
    for (let i = 0; i< ipv4.length; i++) {
      ...
      if (metaData[j][1].includes(ipv4[i][1])) {

      console.log(`True @ metData index ${metaData[j]} : ipv4 index ${ipv4Data} `)
      
    } else {

      console.log(`Negative @ metData index ${metaData[j]} : ipv4 index ${ipv4Data} `)
    }
  console.log(`End of ${i} loops`)
