...
function ComponentChallenge1() {
  ...
  const [parseTest, setParseTest] = useState([]);
  ...
  const handleFetch = async () => {
    ...
    setParseTest(
      data
        ? console.log('JSON parsed correctly / valid')
  ...
  return (
    <div>
      ...
      <div className={toggleOn ? 'Visible' : 'Hidden'}>
        ...
        <hr />
        <h3>
          Checking Parsing of Data: <span className='type'>{parseTest}</span>{' '}
          <span className='validator'>
            {parseTest === 'cors' ? 'Check server proxy port' : 'Valid'}
          </span>
        </h3>
        <hr />
        <h3>
